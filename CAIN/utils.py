...
def PocketVolume(alpha_spheres: List[AlphaSphere]) -> float:
    """
    Calculate the volume of a pocket.
    """
    # Calculate the volume of a pocket.
    # The volume of an alpha sphere is 4/3 * pi * r^3.
    # Don't forget to consider the overlapping volume of the alpha spheres.
    overlap_volume = 0
    for i in range(len(alpha_spheres)):
        for j in range(i+1, len(alpha_spheres)):
            dist = np.linalg.norm(alpha_spheres[i].crd - alpha_spheres[j].crd)
            if dist < alpha_spheres[i].radius + alpha_spheres[j].radius:
                # The two spheres are overlapping.
                # Calculate the overlapping volume.
                r1 = alpha_spheres[i].radius
                r2 = alpha_spheres[j].radius
                d = dist
                h = (r1 + r2 - d) / 2
                overlap_volume += np.pi * h**2 * (3 * (r1 + r2) - h) / 3
    volume = 0
    for alpha_sphere in alpha_spheres:
        volume += 4/3 * np.pi * alpha_sphere.radius**3
...
def gen_ligand_tunnel(alpha_spheres:List[AlphaSphere], pocket:Pocket, d:float) -> Tunnel:
    """
    ...
    while flag:
        for i, alpha_sphere in enumerate(alpha_spheres):
            ...
            if min_distance <= d:
                ...
                flag = False
    tunnel = Tunnel(tunnel_alpha_spheres, pocket.center)
    return tunnel
...
class AlphaCavity:
    ...
    def get_ligand_tunnel(self):
        self.ligand_tunnel = gen_ligand_tunnel(self.AlphaSpheres, self.ligand_pocket, self.tunnel_d)
